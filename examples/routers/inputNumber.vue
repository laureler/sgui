  ...
  <i-article>
    <article>
      ...
      <Demo title="小数">
        ...
        <i-code lang="html" slot="code">{{ code.decimal }}</i-code>
      </Demo>
      <Demo title="格式化展示">
        <div slot="demo">
          <sg-inputNumber  v-model="value2"  :max="10000" :formatter="value => `$ ${value}`.replace(/B(?=(d{3})+(?!d))/g, ',')"
                           :parser="value => value.replace(/$s?|(,*)/g, '')"></sg-inputNumber>
          <sg-inputNumber  :max="100" v-model="value3"  :formatter="value => `${value}%`"
                           :parser="value => value.replace('%', '')"></sg-inputNumber>
        </div>
        <div slot="desc">
...
<script>
  ...
  import Code from '../code/inputNumber';
