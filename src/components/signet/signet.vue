...
<script>
  export default {
    ...
    computed:{
      ctitle(){
        var c = this.name ? this.name : "测试专用章";
        return c;
      },
      otitle(){
        var o =this.organization ? this.organization :'南方数码研发中心团队';
        return o;
      },
      cWidth(){
    ...
    methods:{
      createSeal(id,company,name){
        ...
        //画五角星
        create5star(context,width,height,obj.starRadius,"#f00",0);

        // 绘制印章名称
        context.font = obj.fontN;
    ...
    mounted(){
      ...
      this.createSeal('canvas',this.otitle,this.ctitle);
      var canvas = document.querySelector("canvas");
